...
process_noise_vector = zeros(n_diff,1);
if (user_data_struct.enable_process_noise == 1) && strcmp(user_data_struct.process_noise_flag,'pseudo_white_noise')
    %     process_noise_vector = user_data_struct.chol_Q*randn(n_diff,1);
    process_noise_vector(1) = 0 + 1e-3*sin(100*2*pi*t/user_data_struct.Ts);
    process_noise_vector(2) = 0 + 1e-3*cos(90*2*pi*t/user_data_struct.Ts);
    process_noise_vector(3) = 0 - 1e-3*sin(110*2*pi*t/user_data_struct.Ts);
    process_noise_vector(4) = 0 + 1e-3*sin(90*2*pi*t/user_data_struct.Ts);
    process_noise_vector(5) = 0;
    process_noise_vector(6) = 0;
end

rhs1 = -k2*X(2)*Z(2)                                      + process_noise_vector(1);
rhs2 = -k1*X(2)*X(6) + km1*Z(4) - k2*X(2)*Z(2)            + process_noise_vector(2);
rhs3 =  k2*X(2)*Z(2) + k3*X(4)*X(6) - km3*Z(3)            + process_noise_vector(3);
